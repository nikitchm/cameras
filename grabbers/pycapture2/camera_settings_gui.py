...
from PyQt5.QtWidgets import ( # Changed from PyQt6 to PyQt5
    ...
    QLabel, QSlider, QLineEdit, QPushButton, QFormLayout, QMessageBox, QComboBox
)
...
class SettingsWindow(QDialog): # Inherit from QDialog
    ...
    def init_ui(self):
        ...
        # --- offsetX Control ---
        self.offsetX_input = QLineEdit()
        self.offsetX_input.setValidator(QIntValidator(1, 4096)) # Assuming common max height
        self.offsetX_input.editingFinished.connect(lambda: self._validate_and_update_value(self.offsetX_input))
        form_layout.addRow("offsetX (pixels):", self.offsetX_input)

        # --- offsetY Control ---
        self.offsetY_input = QLineEdit()
        self.offsetY_input.setValidator(QIntValidator(1, 4096)) # Assuming common max height
        self.offsetY_input.editingFinished.connect(lambda: self._validate_and_update_value(self.offsetY_input))
        form_layout.addRow("offsetY (pixels):", self.offsetY_input)
        ...
        self.mode_selector.addItems(["0", "1"])
        form_layout.addRow("Mode:", self.mode_selector)
    ...
    def load_current_settings(self):
        ...
        if self._initial_props:
            ...
            self.offsetX_input.setText(str(self._initial_props.offsetX))
            self.offsetY_input.setText(str(self._initial_props.offsetY))
            self.fps_input.setText(f"{self._initial_props.fps:.1f}") # Format to 1 decimal place
            if 'mode' in self._initial_props.other:
                self.mode_selector.setCurrentText(str(self._initial_props.other['mode']))
            # Brightness
            ...
            self.offsetX_input.setText("0")
            self.offsetY_input.setText("0")
            self.fps_input.setText("45.0")
            self.mode_selector.setCurrentText("0")
    ...
    def apply_settings(self):
        ...
        try:
            ...
            new_offsetX = int(self.offsetX_input.text())
            new_offsetY = int(self.offsetY_input.text())
            new_fps = float(self.fps_input.text())
            new_brightness = self.brightness_slider.value() if self.brightness_slider.isEnabled() else -1
            new_mode = int(self.mode_selector.currentText())
...
if __name__ == '__main__':
    ...
    class MockCameraGrabber(CameraGrabberInterface):
        def __init__(self):
            self._is_open = False
            self._props = CameraProperties(width=1024,
                                           height=1024,
                                           offsetX=0,
                                           offsetY=0,
                                           fps=45.0,
                                           brightness=100,
                                           other={'mode':1}) # Initial mock properties
    ...
    initial_camera_props = mock_grabber.open(0, CameraProperties(width=1024,
                                    height=1024,
                                    offsetX=0,
                                    offsetY=0,
                                    fps=45.0,
                                    brightness=100,
                                    other={'mode':1})) # Simulate opening a camera
    ...
    def on_settings_applied(new_props: CameraProperties):
        ...
        # This it a dummy plug for the method defined in the main (parent) window,
        # where settings_applied() should be connected to an appropriate method.
        # There, you would restart the camera with these new_props. Smth like this:
